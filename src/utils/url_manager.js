export function fragmentToURLParameters(url) {
    const url_href = typeof url === 'object' ? url.href : new URL(url).href;

    // Odoo url can manage fragment for some parameter, need to merge fragment and classic parameter
    return new URL(url_href.replace(/#/g, url_href.includes('?') ? '&' : '?'));
}
...
export async function getModelAndID_fromURL(url, model = undefined) {
    url = typeof url === 'object' ? url : new URL(url);
