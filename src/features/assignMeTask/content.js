...
import { fragmentToURLParameters, getProjectTaskID_fromURL } from '../../utils/url_manager.js';
...
export default class AssignMeTaskContentFeature extends ProjectTaskShareContentFeature {
    ...
    async addUserToTaskAssignees(task, userID, callback) {
        const taskID = await getProjectTaskID_fromURL(
            fragmentToURLParameters(window.location.href)
        );
        if (task.id != taskID)
            throw new Error(
